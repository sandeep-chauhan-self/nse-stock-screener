...
class PerformanceMode(Enum):
    ...
    BALANCED = "balanced"      # Balance accuracy and speed
...
class StressTestConfig:
    ...
    expected_ranges: Dict[str, Dict[str, float]] = field(default_factory=lambda: {
        "RSI": {"min": 0, "max": 100},
        "ATR": {"min": 0, "max": 50},  # % of price
        "ADX": {"min": 0, "max": 100},
        "MACD": {"min": -10, "max": 10}  # Relative to price
    })
...
def get_development_config() -> IndicatorEngineConfig:
    ...
    return IndicatorEngineConfig(
        ...
        enable_numba=False,  # Easier debugging
        performance_mode=PerformanceMode.ACCURACY,
        enable_caching=False,  # Fresh results for testing
        strict_validation=True,
...
def get_performance_config() -> IndicatorEngineConfig:
    ...
    return IndicatorEngineConfig(
        ...
        continue_on_error=True,
        log_computation_errors=False,  # Reduce logging overhead
        min_data_quality_threshold=0.6,  # More permissive
