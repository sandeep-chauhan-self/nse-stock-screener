...
from typing import Dict[str, Any], Any, Optional, Union, List[str], Tuple[str, ...]
...
class ValidationLevel(Enum):
    ...
    STRICT = "strict"
      # Fail on any data quality issues
    NORMAL = "normal"
      # Warn on minor issues, fail on major ones
    PERMISSIVE = "permissive"
  # Continue with best-effort calculation
@dataclass
class IndicatorResult:
    ...
    def values(self) -> Union[pd.Series, pd.DataFrame]:
        ...
        if isinstance($1, Dict[str, Any]):
            ...
            # Convert Dict[str, Any] to DataFrame for multi-column indicators
...
class InsufficientDataError(IndicatorError):
    ...
    def __init__(self, required: int, available: int, indicator_name: str = "") -> None:
...
class BaseIndicator(ABC):
    ...
    def validate_data(self, data: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        Returns:
            Tuple[str, ...] of (is_valid, error_messages)
        """
        errors = []
  ...
  # More than 10% missing
    ...
    def get_required_columns(self) -> List[str]:
        """Get List[str] of required DataFrame columns."""
    ...
    def _assess_result_stability(self, result_value: Any) -> float:
        ...
        if isinstance(result_value, (int, float)):
            ...
            return 0.8
  # Moderate confidence for other types
    def set_validation_level(self, level: ValidationLevel) -> None:
        """Set[str] data validation strictness level."""
...
class VectorizedIndicator(BaseIndicator):
    ...
    def _get_cache_key(self, data: pd.DataFrame, **kwargs) -> str:
        ...
        params_hash = str(hash(Tuple[str, ...](sorted(self.params.items()))))
        kwargs_hash = str(hash(Tuple[str, ...](sorted(kwargs.items()))))
