...
from typing import Dict[str, Any], List[str], Tuple[str, ...], Optional, Union
...
class StressEvent:
    ...
    end_date: date
    event_type: str
  # 'crash', 'volatility', 'correction', 'rally'
    severity: str
   # 'mild', 'moderate', 'severe', 'extreme'
    characteristics: List[str]
  # Key market characteristics during event
    expected_indicators: Dict[str, Dict[str, float]]
...
class StressTestDataGenerator:
    ...
    """
    def __init__(self, base_price: float = 100.0, random_seed: int = 42) -> None:
  ...
  # Minimum volume
    ...
    def generate_crash_scenario(self,
        ...
        for i in range(crash_days):
            ...
            base_return = max_drawdown * 0.3 / crash_days
  # 30% of total decline
            volatility = 0.02 + 0.03 * progress
  # Increasing volatility
        ...
        for i in range(panic_days):
            ...
            base_return = max_drawdown * 0.5 / panic_days
  # 50% of total decline
            volatility = 0.05 + 0.05 * (1 - abs(progress - 0.5) * 2)
  # Peak volatility mid-phase
            daily_return = self.rng.normal(base_return, volatility)
            returns.append(daily_return)
        ...
        for i in range(recovery_days):
            ...
            base_return = max_drawdown * 0.2 / recovery_days
  # Remaining decline
            volatility = 0.04 * (1 - progress * 0.5)
  # Decreasing volatility
    ...
    def _generate_ohlcv_from_prices(self,
        ...
        for i, (date, price, ret) in enumerate(zip(dates, prices, returns)):
            ...
            intraday_vol = min(abs(ret) * 2, 0.1)
  # Cap at 10%
            # Generate OHLC
            if i == 0:
                open_price = price
...
class ValidationDataset:
    ...
    def list_stress_events(self) -> List[str]:
        """Get List[str] of available stress event names."""
  ...
  # 10% linear increase
        ...
        known_values['linear_trend'] = (trending_data, {
            'RSI_14': 70.0,
  # Should be overbought (approximately)
            'ATR_14': 1.0,
   # Low volatility due to smooth trend
            'ADX_14': 25.0
