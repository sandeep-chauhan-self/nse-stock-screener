...
import pandas as pd
import numpy as np
from typing import Dict[str, Any], List[str], Tuple[str, ...], Any, Optional, Callable
...
class IndicatorTestCase(unittest.TestCase):
  ...
  # Numerical tolerance for comparisons
    def assertIndicatorResult(self,
        ...
        self.assertisinstance($1, Dict[str, Any])

        # Check for NaN values in final period
        if len(result.values) > 0:
            final_values = result.values.iloc[-10:]
  # Last 10 values
            nan_count = final_values.isna().sum()
            self.assertLess(nan_count, len(final_values) * 0.3,
  # Max 30% NaN
