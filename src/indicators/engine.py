...
class IndicatorEngine:
    ...
    def _create_basic_indicator_set(self) -> None:
        """Create a basic set of indicators as fallback."""
        try:
            ...
            logger.info("Created basic indicator set as fallback")
    ...
    def load_config_file(self, config_path: Union[str, Path]) -> None:
        ...
        try:
            ...
            if config_path.exists():
                ...
                self._load_default_indicators()  # Reload with new config
    ...
    def set_validation_level(self, level: ValidationLevel) -> None:
        """Set validation level for all indicators."""
        for indicator in self._indicators.values():
            indicator.set_validation_level(level)
        logger.debug(f"Set validation level to {level.value}")
    ...
    def _generate_cache_key(self, symbol: str, data: pd.DataFrame) -> str:
        ...
        indicator_hash = str(hash(tuple(self._indicators.keys())))
    ...
    def _cleanup_cache(self) -> None:
        ...
        if len(self._result_cache) > self.config.max_cache_size * 1.2:
            ...
            keys_to_remove = list(self._result_cache.keys())[:-self.config.max_cache_size]
    ...
    def list_indicators(self) -> List[str]:
        """Get list of loaded indicator names."""
    ...
    def calculate_indicators(self, 
        ...
        """
        Convenience method to calculate a set of indicators on data.
