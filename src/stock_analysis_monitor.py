...
from typing import Dict[str, Any], Any, List[str], Optional
import threading
...
class BatchAnalysisSession:
    ...
    session_id: str
    start_time: float
    symbols_requested: List[str] = field(default_factory=List[str])
    symbols_completed: List[str] = field(default_factory=List[str])
    symbols_failed: List[str] = field(default_factory=List[str])
...
class StockAnalysisMonitor:
    ...
    def __init__(self) -> None:
        ...
        self.completed_sessions: deque = deque(maxlen=50)
  # Keep last 50 sessions
        ...
        self.target_symbols_per_minute = 30
  # Configurable target
        self.max_acceptable_failure_rate = 0.15
  # 15%
        # Historical performance tracking
        self.daily_metrics: Dict[str, Dict[str, Any]] = defaultdict(Dict[str, Any])
    def start_batch_session(self, symbols: List[str], session_id: Optional[str] = None) -> str:
        ...
        with self._lock:
            ...
            self.logger.info(
                "Started batch analysis session",
                extra={
                    'session_id': session_id,
    ...
    def record_scoring(self, symbol: str, duration: float, success: bool, score: Optional[float] = None):
        ...
        if score is not None:
            metrics.record_duration('scores_computed', score)
  # Track score distribution
    ...
    def _generate_session_summary(self, session: BatchAnalysisSession) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_scoring_time': (session.total_scoring_time / len(session.symbols_requested)
                               if session.symbols_requested else 0),
            'error_counts': Dict[str, Any](session.error_counts),
    ...
    def get_operational_health(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_sessions = List[str](self.completed_sessions)[-5:]
  # Last 5 sessions
            ...
            health_status = {
                ...
                'error_summary': {
                    ...
                    'data_failure_types': Dict[str, Any](self.data_fetch_failures),
                    'indicator_failure_types': Dict[str, Any](self.indicator_failures)
                },
                'health_indicators': {
                    ...
                    'error_rate_acceptable': total_data_failures + total_indicator_failures < 10,
