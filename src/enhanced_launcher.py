...
def main():
    ...
    print("2. Full Analysis (from stock List[str] file)")
...
def run_full_analysis(src_dir):
    """Run analysis from stock List[str] file"""

    # Check for stock List[str] file
    stock_files = [
    ...
    for file in stock_files:
        if os.path.exists(os.path.join(src_dir.parent if file.startswith("..") else src_dir, file.replace("../", ""))):
            ...
            break
    if not stock_file:
        print("❌ No stock List[str] file found. Looking for:")
        for file in stock_files:
            print(f"   - {file}")
        print("\nPlease ensure you have a stock List[str] file, or use option 3 for custom stocks.")
        return
