...
class EnhancedEarlyWarningSystem:
    ...
    def __init__(self, custom_stocks: Optional[List[str]] = None,
        ...
        Args:
            custom_stocks: List[str] of custom stock symbols
    ...
    def _load_stock_symbols(self, custom_stocks: Optional[List[str]],
        ...
        if custom_stocks:
            logger.info(f"Using custom stock List[str]: {len(custom_stocks)} stocks",
  ...
  # Annualized volatility
            ...
            # Determine regime
            if volatility > 0.25:
  # 25% annual volatility threshold
  ...
  # 2x ATR stop
                ...
                scoring_result['risk_management'] = {
                    ...
                    'risk_reward_ratio': round(abs(entry_price - stop_loss) / (entry_price * 0.025), 2)
  # Assume 2.5% TP
                }
                ...
                return None
    def filter_and_rank_results(self, results: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def generate_enhanced_chart(self, symbol: str, indicators: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Error generating enhanced chart for {symbol}: {e}",
                        ...
                        exc_info=True)
            return None
    def save_enhanced_reports(self, categorized_results: Dict[str, List[Dict[str, Any]]],
    ...
    def display_analysis_results(self, categorized_results: Dict[str, List[Dict[str, Any]]],
        ...
        if categorized_results['HIGH']:
            ...
            high_df = pd.DataFrame([
                ...
                for r in categorized_results['HIGH'][:15]
  # Top 15
            ])
        ...
        if categorized_results['MEDIUM']:
            ...
            medium_df = pd.DataFrame([
                ...
                for r in categorized_results['MEDIUM'][:10]
  # Top 10
            ])
    ...
    def generate_charts_for_top_stocks(self, categorized_results: Dict[str, List[Dict[str, Any]]]):
  ...
  # Limit to avoid too many API calls
        for result in categorized_results['HIGH'][:5]:
  # Top 5 high prob
  ...
  # Rate limiting
        ...
        for result in categorized_results['MEDIUM'][:5]:
  # Top 5 medium prob
            if chart_count >= max_charts:
                break
            symbol = result['symbol']
  ...
  # Rate limiting
    ...
    def run_backtest_analysis(self, start_date: Optional[datetime] = None,
        ...
        for result in self.analysis_results:
            if result['composite_score'] >= 45:
  # Only test medium+ signals
                symbol = result['symbol']
                if symbol not in signals_data:
                    signals_data[symbol] = []
                signals_data[symbol].append({
                    'date': datetime.now().date(),
  # In real implementation, use signal date
                    'signal_data': result
                })
...
if __name__ == "__main__":
    ...
    # Create custom stock List[str] if provided
    custom_stocks = None
    if args.stocks:
