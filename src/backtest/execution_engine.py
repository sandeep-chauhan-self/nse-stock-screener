...
class Fill:
    ...
    total_cost: float = 0.0  # Including commission
...
class IndianMarketExecutionModel(ExecutionModel):
    ...
    def execute_market_order(self, order: Order, market_data: MarketData,
        ...
        if order.side == OrderSide.BUY:
            ...
            execution_price = base_price + slippage_per_share  # Additional slippage
        else:
            base_price = bid  # Sell at bid
            execution_price = base_price - slippage_per_share  # Slippage reduces price
    ...
    def execute_limit_order(self, order: Order, market_data: MarketData,
        ...
        if not order.price:
            return fills  # No limit price specified
        ...
        queue_position_factor = self.rng.uniform(0.7, 1.0)  # 70-100% of order filled
    ...
    def _check_limit_execution(self, order: Order, bid: float, ask: float) -> Dict[str, Any]:
        ...
        if order.side == OrderSide.BUY:
            ...
            if ask <= order.price:
                can_execute = True
                execution_price = min(ask, order.price)  # Price improvement possible
        else:
            # Sell limit: can execute if bid >= limit price
            if bid >= order.price:
                can_execute = True
                execution_price = max(bid, order.price)  # Price improvement possible
    ...
    def execute_order(self, order: Order, market_data: MarketData,
        ...
        Returns:
            List of fills generated from this execution attempt
        """
