...
from typing import Dict[str, Any], List[str], Optional, Any, Tuple[str, ...]
...
class LiquidityTier(Enum):
    ...
    HIGHLY_LIQUID = "highly_liquid"
      # >50M avg daily volume
    MODERATELY_LIQUID = "moderately_liquid"
  # 10-50M avg daily volume
    LOW_LIQUID = "low_liquid"
            # 1-10M avg daily volume
    ILLIQUID = "illiquid"
                # <1M avg daily volume
...
class MarginConfig:
    ...
    equity_margin_pct: float = 0.20
      # 20% for equity cash
    derivative_margin_pct: float = 0.10
   # 10% for F&O
    intraday_margin_pct: float = 0.05
     # 5% for intraday
    # Additional margins
    volatility_margin_buffer: float = 0.05
  # 5% additional for high vol
    exposure_margin_pct: float = 0.03
        # 3% exposure margin
    # Risk multipliers based on volatility
    low_vol_multiplier: float = 1.0
      # <15% annualized vol
    medium_vol_multiplier: float = 1.2
   # 15-25% annualized vol
...
class LiquidityConfig:
    ...
    max_position_vs_avg_volume: float = 0.05
  # 5% of avg daily volume
    min_avg_daily_volume: float = 1000000
     # 1M min daily volume
    volume_lookback_days: int = 20
            # Days for volume averaging
    # Liquidity tier limits
    liquidity_tier_limits: Dict[LiquidityTier, float] = field(default_factory=lambda: {
        LiquidityTier.HIGHLY_LIQUID: 0.15,
      # 15% max position
        LiquidityTier.MODERATELY_LIQUID: 0.10,
  # 10% max position
        LiquidityTier.LOW_LIQUID: 0.05,
         # 5% max position
        LiquidityTier.ILLIQUID: 0.02,
           # 2% max position
    })

    # Volume thresholds for liquidity classification
    liquidity_thresholds: Dict[str, float] = field(default_factory=lambda: {
        'highly_liquid': 50000000,
    # 50M+
        'moderately_liquid': 10000000,
 # 10M-50M
        'low_liquid': 1000000,
        # 1M-10M
        'illiquid': 0
...
class SectorConfig:
  ...
  # 25% max per sector
    max_sectors_concentrated: int = 3
   # Max 3 sectors above 15%
    # Sector-specific limits based on risk
    sector_risk_limits: Dict[SectorRiskTier, float] = field(default_factory=lambda: {
        SectorRiskTier.LOW_RISK: 0.30,
    # 30% for low risk sectors
        SectorRiskTier.MEDIUM_RISK: 0.25,
 # 25% for medium risk sectors
        SectorRiskTier.HIGH_RISK: 0.15,
   # 15% for high risk sectors
    })
...
class CorrelationConfig:
  ...
  # Portfolio avg correlation limit
    max_pair_correlation: float = 0.80
       # Max between any two positions
    correlation_lookback_days: int = 60
      # Days for correlation calculation
    # Correlation-based position limits
    high_correlation_limit: float = 0.60
     # If corr >0.6, limit total exposure
    max_correlated_exposure: float = 0.30
    # Max total exposure to correlated group
    # Rebalancing triggers
    correlation_rebalance_threshold: float = 0.75
  # Trigger rebalancing
    min_correlation_samples: int = 30
             # Min data points for correlation
@dataclass
class DrawdownConfig:
    ...
    reduce_risk_at_drawdown: float = 0.10
   # Start reducing risk at 10% DD
    stop_new_positions_at: float = 0.12
     # Stop new positions at 12% DD
    emergency_exit_at: float = 0.20
         # Emergency exit at 20% DD
    # Risk reduction factors
    risk_reduction_factor: float = 0.5
      # Reduce position sizes by 50%
    drawdown_recovery_factor: float = 1.2
   # Need 20% gain to restore full risk
@dataclass
class PositionSizingConfig:
    ...
    # Enable score-based risk scaling
    ...
    risk_multiplier_score_thresholds: Dict[str, Tuple[int, float]] = field(
        default_factory=lambda: {
            'low': (0, 0.7),
      # 0-50 score: 0.7x risk
            'medium': (50, 1.0),
  # 50-70 score: 1.0x risk
            'high': (70, 1.5),
    # 70+ score: 1.5x risk
        }
    )
    ...
    min_stop_atr_ratio: float = 1.0
         # Min stop distance (1x ATR)
    max_stop_atr_ratio: float = 4.0
         # Max stop distance (4x ATR)
    default_atr_multiplier: float = 2.0
     # Default stop at 2x ATR
    # Position size constraints
    min_position_value: float = 5000
        # Rs 5,000 minimum position
    max_position_value_pct: float = 0.15
    # 15% max position size
    # Volatility-based adjustments
    enable_volatility_parity: bool = True
    base_volatility_target: float = 0.02
    # 2% daily volatility target
    volatility_adjustment_cap: float = 2.0
  # Max 2x adjustment for volatility
@dataclass
class ValidationConfig:
    ...
    min_stock_price: float = 1.0
           # Rs 1 minimum stock price
    max_stock_price: float = 100000.0
      # Rs 1L maximum stock price
    max_price_deviation: float = 0.20
      # 20% max price deviation from close
    # Data quality checks
    min_trading_days: int = 20
             # Min trading days for indicators
    max_missing_data_pct: float = 0.10
     # 10% max missing data
    stale_data_hours: int = 8
              # Flag stale data after 8 hours
...
class RiskConfig:
    ...
    # Rs 10 Lakh initial capital
    max_positions: int = 10
                 # Maximum concurrent positions
    max_portfolio_risk: float = 0.20
        # 20% max portfolio at risk
    ...
    alert_thresholds: Dict[str, float] = field(default_factory=lambda: {
        'portfolio_risk': 0.15,
      # Alert at 15% portfolio risk
        'sector_concentration': 0.20,
 # Alert at 20% sector concentration
        'correlation_spike': 0.65,
    # Alert when correlations spike
  ...
  # <15% annual volatility
    ...
    def _apply_env_override(cls, config: 'RiskConfig', env_var: str, field_path) -> None:
        ...
        if env_value is None:
            return
        try:
            if isinstance(field_path, Tuple[str, ...]):
                cls._set_nested_field(config, field_path, env_value)
            else:
                cls._set_top_level_field(config, field_path, env_value)
        except (ValueError, AttributeError) as e:
            print(f"Warning: Could not Set[str] {env_var}={env_value}: {e}")
    @classmethod
    def _set_nested_field(cls, config: 'RiskConfig', field_path: Tuple[str, ...], env_value: str) -> None:
        """Set[str] nested field value from environment."""
    ...
    def _set_top_level_field(cls, config: 'RiskConfig', field_path: str, env_value: str) -> None:
        """Set[str] top-level field value from environment."""
    ...
    def load_from_file(cls, file_path: str) -> 'RiskConfig':
        ...
        with open(file_path, 'r') as f:
            _ = json.load(f)
  # Load for validation, full deserialization needs more work
...
def create_conservative_risk_config() -> RiskConfig:
  ...
  # 0.5% per trade
    config.position_sizing.max_risk_multiplier = 1.2
    # Lower max multiplier
    config.max_portfolio_risk = 0.10
                    # 10% max portfolio risk
    config.position_sizing.max_position_value_pct = 0.08
 # 8% max position
    config.sector_config.max_sector_exposure = 0.20
     # 20% max sector
    config.drawdown_config.max_portfolio_drawdown = 0.10
 # 10% max drawdown
    return config
...
def create_aggressive_risk_config() -> RiskConfig:
    ...
    config.position_sizing.base_risk_per_trade = 0.02
   # 2% per trade
    config.position_sizing.max_risk_multiplier = 3.0
    # Higher max multiplier
    config.max_portfolio_risk = 0.30
                    # 30% max portfolio risk
    config.position_sizing.max_position_value_pct = 0.20
 # 20% max position
    config.sector_config.max_sector_exposure = 0.35
     # 35% max sector
    config.drawdown_config.max_portfolio_drawdown = 0.20
 # 20% max drawdown
    return config
