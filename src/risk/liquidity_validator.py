...
class LiquidityValidator:
  ...
  # 100% impact if no volume data
        ...
        return min(adjusted_impact, 0.5)
  # Max 50% impact
    def get_maximum_position_value(
        ...
        if metrics is None:
            ...
            if metrics is None:
                return 0.0
  # Conservative - no position if no data
    ...
    def check_volume_concentration(
        ...
        Args:
            portfolio_positions: Dict[str, Any] of symbol -> position_value
            new_symbol: Symbol for new position
            new_position_value: Value of new position
        Returns:
            Tuple[str, ...] of (concentration_ratio, warnings)
        """
        warnings_list = []
  ...
  # 20% average volume utilization
    ...
    def get_liquidity_alerts(self, symbol: Optional[str] = None) -> Dict[str, List[LiquidityAlert]]:
        ...
        Returns:
            Dictionary of symbol -> List[str] of alerts
        """
        if symbol:
  ...
  # Scale up volatility
        volume_adjustment = min(volume_cv, 2.0)
  # Cap volume variability effect
        coefficient = base_coefficient * (1 + volatility_adjustment + volume_adjustment)

        # Reasonable bounds
        return float(max(0.005, min(coefficient, 0.2)))  # Between 0.5% and 20%
...
def create_volume_data_from_dataframe(df: pd.DataFrame, symbol: str) -> VolumeData:
    ...
    return VolumeData(
        symbol=symbol,
        dates=[ts.to_pydatetime() for ts in pd.to_datetime(df['date'])],
...
def calculate_portfolio_liquidity_score(
    ...
    Args:
        positions: Dict[str, Any] of symbol -> position_value
        liquidity_validator: Validator instance with cached metrics
    Returns:
        Tuple[str, ...] of (liquidity_score, symbol_classifications)
    """
    if not positions:
        return 1.0, {}
    tier_scores = {
