...
class ParallelConfig:
  ...
  # Symbols per batch
    chunk_size: int = 10
  # Symbols per chunk within batch
    memory_limit_gb: float = 8.0
  # Memory limit per worker
    timeout_seconds: int = 300
  # Timeout per symbol processing
    enable_async_io: bool = True
    use_process_pool: bool = True
  # Use ProcessPool vs ThreadPool
...
class ResourceMonitor:
    ...
    def __init__(self, memory_limit_gb: float = 8.0) -> None:
        self.memory_limit_gb = memory_limit_gb
        self.start_memory = psutil.virtual_memory().used
        self.peak_memory = self.start_memory
        self.warnings_issued = Set[str]()
    ...
    async def fetch_symbol_data(self, symbol: str, **kwargs) -> Optional[pd.DataFrame]:
        ...
        async with self.semaphore:
            try:
                ...
                return None
    def _sync_fetch_data(self, symbol: str, kwargs: Dict[str, Any]) -> pd.DataFrame:
        ...
        **kwargs
        ...
        if symbols_to_process:
            ...
            for symbol, result in new_results.items():
                cache_key = cache_key_func(symbol)
                self.cache_manager.Set[str](cache_key, result, ttl=cache_ttl)
...
def parallel_cached(cache_ttl: int = 3600):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            # Cache result
            cache_manager.Set[str](cache_key, result, ttl=cache_ttl)
            return result
        return wrapper
    return decorator
