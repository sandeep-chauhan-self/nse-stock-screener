...
class ProfilingConfig:
    ...
    enable_line_profiling: bool = False  # Expensive, use sparingly
    ...
    sample_rate: float = 0.1  # Profile 10% of operations
    min_execution_time: float = 0.001  # Only profile functions taking > 1ms
    ...
    regression_threshold: float = 1.5  # Alert if 50% slower than baseline
    baseline_window_size: int = 100  # Use last 100 measurements for baseline
...
class ProfileResult:
    ...
    memory_trace: Optional[List] = None
    line_profile: Optional[Dict] = None
...
class PerformanceProfiler:
    ...
    def cached_computation(self, cache_key: Optional[str] = None):
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                # Cache result
                self.cache.set(key, result, computation_time)
    ...
    def _save_profile_data(self, result: ProfileResult):
        ...
        try:
            ...
            filename = f"{result.function_name}_{timestamp}.prof"
    ...
    def _cleanup_old_profiles(self):
        ...
        try:
            ...
            for file_path in self.output_dir.glob("*.prof"):
    ...
    def _check_performance_regression(self, function_name: str, execution_time: float):
        ...
        baseline_times = list(self.baseline_metrics[function_name])
        
        if len(baseline_times) < 10:  # Need sufficient baseline data
            return
    ...
    def get_function_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        for func_name in self.function_call_counts.keys():
            ...
            recent_times = [r.execution_time for r in self.profile_results 
                          if r.function_name == func_name][-50:]  # Last 50 calls
    ...
    def get_optimization_recommendations(self) -> Dict[str, Any]:
        ...
        slow_functions = [
            ...
            if stats["avg_time"] > 0.1  # Functions taking > 100ms on average
        ]
        slow_functions.sort(key=lambda x: x[1]["total_time"], reverse=True)
        
        for name, stats in slow_functions[:5]:  # Top 5 slow functions
        ...
        if not isinstance(memory_analysis, dict) or "error" in memory_analysis:
            pass  # No memory data available
        ...
        for name, stats in frequent_functions[:3]:  # Top 3 frequent functions
            if stats["avg_time"] > 0.01:  # If each call takes > 10ms
                recommendations.append({
                    "type": "optimization",
                    "priority": "medium",
                    "function": name,
...
if __name__ == "__main__":
    ...
    config = ProfilingConfig(
        sample_rate=1.0,  # Profile all calls for testing
    ...
    for i in range(5):
        result = fibonacci(20)  # Should be cached after first call
