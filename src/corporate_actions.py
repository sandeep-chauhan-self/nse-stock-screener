...
import logging
from typing import Dict[str, Any], Optional, Tuple[str, ...], Any
import numpy as np
import pandas as pd
...
class CorporateActionHandler:
  ...
  # 5 minutes cache for repeated requests
    ...
    def fetch_raw_and_adjusted_data(self, symbol: str, period: str = "1y",
        ...
        Returns:
            Tuple[str, ...] of (raw_data, adjusted_data) DataFrames
        """
        try:
            ticker = yf.Ticker(symbol)
...
def validate_corporate_action_consistency(data: pd.DataFrame) -> Dict[str, Any]:
  ...
  # 50% single-day changes
        ...
        if 'Volume' in data.columns:
            ...
            volume_spikes = volume_changes > 2.0
  # 200% volume increases
...
if __name__ == "__main__":
    ...
    if adjusted_data is not None:
        ...
        print("Corporate action analysis:")
        ...
        if ca_events['events']:
            print("  Recent events:")
            for event in ca_events['events'][-3:]:
  # Show last 3 events
