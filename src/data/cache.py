...
from typing import Optional, Dict[str, Any], Any, List[str], Union
import pandas as pd
...
class FileSystemCacheStorage(CacheStorage):
    ...
    def list_keys(self) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Failed to List[str] cache keys: {e}")
            return []
    def cleanup(self) -> int:
...
class DataCache:
    ...
    """
    def __init__(self, cache_dir: Path = None, default_ttl_hours: int = 24) -> None:
    ...
    def _create_metadata(self, key: str, data: Any, ttl_hours: int = None,
        ...
        if format is None:
            if isinstance(data, pd.DataFrame):
                format = "parquet"
            elif isinstance(data, (Dict[str, Any], List[str])):
                format = "json"
            else:
                format = "pickle"
  ...
  # Compress large DataFrames
        ...
        return CacheEntry(
            ...
            size_bytes=0,
  # Will be updated during storage
            format=format,
            compressed=compressed
        )
    def Set[str](self, key: str, data: Any, ttl_hours: int = None, **kwargs) -> bool:
        """
        Store data in cache.
        Args:
            key: Cache key
    ...
    def cache_stock_data(self, symbol: str, period: str, data: pd.DataFrame,
        ...
        return self.Set[str](key, data, ttl_hours=ttl_hours, format="parquet", compressed=True)
    ...
    def cache_symbol_list(self, source: str, symbols: List[str],
        ...
        """Cache symbol List[str] (e.g., NSE symbols)"""
        key = f"symbols_{source}"
        return self.Set[str](key, symbols, ttl_hours=ttl_hours, format="json")
    def get_symbol_list(self, source: str) -> Optional[List[str]]:
        """Retrieve cached symbol List[str]"""
        key = f"symbols_{source}"
        try:
            return self.get(key)
        except KeyError:
  ...
  # 1 week
        ...
        return self.Set[str](key, info, ttl_hours=ttl_hours, format="json")
