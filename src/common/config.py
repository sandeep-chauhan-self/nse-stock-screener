...
class DataConfig:
    ...
    max_missing_data_pct: float = 0.1  # 10%
...
class ScoringConfig:
    ...
    volume_profile_weight: float = 15.0  # Adjusted to make sum = 100
    weekly_bonus_weight: float = 10.0  # This is a bonus, not part of the base 100
...
class RiskConfig:
    ...
    risk_multiplier_threshold: float = 70.0  # Score threshold for risk scaling
    max_risk_multiplier: float = 1.5

    # Stop loss configuration
    atr_stop_multiplier: float = 2.0
    min_stop_loss_pct: float = 0.02  # 2%
    max_stop_loss_pct: float = 0.08  # 8%
    ...
    max_single_stock_exposure: float = 0.20  # 20% single stock

    # Liquidity requirements
    min_daily_volume: int = 100000  # Minimum daily volume
    min_market_cap: float = 1000000000  # 1B minimum market cap
        ...
        for env_key, env_value in os.environ.items():
            if env_key.startswith(env_prefix):
                ...
                try:
                    ...
                    print(f"Warning: Could not set config key '{config_key}' from environment: {e}")
