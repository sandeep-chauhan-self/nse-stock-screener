...
from typing import Dict[str, Any], Optional, Tuple[str, ...], List[str]
import numpy as np
import pandas as pd
...
class VolumeThresholdConfig:
  ...
  # 99th percentile for extreme detection
    high_percentile: float = 95.0
     # 95th percentile for high detection
  ...
  # 1 year of trading days
    ...
    def __post_init__(self):
      ...
      # 2.5 * 2.0 = 5.0x (easier in bull market)
                MarketRegime.SIDEWAYS: 2.0,
     # 3.0 * 2.0 = 6.0x (neutral baseline)
                MarketRegime.BEARISH: 2.5,
      # 4.0 * 2.5 = 10.0x (harder in bear market)
                MarketRegime.HIGH_VOLATILITY: 2.0
  # 5.0 * 2.0 = 10.0x (high vol needs confirmation)
            }
...
class VolumeThresholdCalculator:
    ...
    """
    def __init__(self, config: VolumeThresholdConfig = None) -> None:
        """
    ...
    def calculate_thresholds_multiplier_based(self, regime: MarketRegime) -> Tuple[float, float]:
        ...
        Returns:
            Tuple[str, ...] of (high_threshold, extreme_threshold)
        """
    ...
    def calculate_thresholds_percentile_based(self,
        ...
        Returns:
            Tuple[str, ...] of (high_threshold, extreme_threshold) or (None, None) if insufficient data
        """
    ...
    def get_volume_score_and_level(self,
        ...
        Returns:
            Tuple[str, ...] of (score, level_description)
        """
