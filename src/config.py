...
class SystemConfig:
  ...
  # Initial capital (10 Lakh)
    ...
    max_position_size: float = 0.10
       # 10% max per position (as fraction of capital)
    ...
    max_positions: int = 10
               # Maximum concurrent positions (canonical name)
    ...
    breakeven_trigger_ratio: float = 1.5
      # Move to breakeven after 1.5x risk
 ...
 # Trail at 1x ATR
    ...
    brokerage_rate: float = 0.0003
       # 0.03% brokerage per side
    stt_rate: float = 0.00025
           # 0.025% STT (Securities Transaction Tax)
    exchange_charges: float = 0.0000345
  # 0.00345% exchange charges
    gst_rate: float = 0.18
              # 18% GST on brokerage + exchange charges
    stamp_duty_rate: float = 0.00015
    # 0.015% stamp duty (buy side only)
    # Legacy combined rate (for backward compatibility)
    transaction_cost: float = 0.0005
    # Calculated: ~0.05% total per side
    # Slippage Model (Price Impact)
    slippage_model: str = "adaptive"
     # "fixed", "adaptive", "liquidity_based"
    base_slippage_bps: float = 2.0
      # Base slippage in basis points (0.02%)
    slippage_per_share: float = 0.05
    # Fixed slippage in Rs per share
    market_impact_factor: float = 0.1
   # Market impact coefficient
    # Execution Model Parameters
    partial_fill_enabled: bool = False
  # Enable partial fill simulation
    min_fill_ratio: float = 0.5
        # Minimum % of order that gets filled
    liquidity_impact_threshold: float = 0.01
  # Order size % of volume for impact
    max_spread_bps: float = 50.0
        # Max bid-ask spread in basis points
    # Backtesting Parameters
    max_holding_days: int = 30
           # Maximum days to hold position
    min_score_threshold: int = 45
        # Minimum composite score for entry
    walk_forward_window: int = 252
       # 1 year training window (trading days)
    test_window: int = 63
               # 3 months test window (trading days)
    # Data & Analysis
    batch_size: int = 50
                # Stocks per batch in analysis
    request_timeout: int = 10
           # Seconds between API requests
    data_cache_days: int = 1
            # Days to cache data
    min_trading_volume: int = 100000
    # Minimum daily volume for analysis
    # Scoring & Indicators
    regime_detection_window: int = 50
   # Days for market regime detection
    volatility_window: int = 20
         # Days for volatility calculations
    volume_threshold_multiplier: float = 1.5
  # Volume threshold for extremes
    ...
    # Maximum risk multiplier cap
    min_risk_multiplier: float = 0.25
   # Minimum risk multiplier floor
    risk_multiplier_high_score: float = 1.8
   # Multiplier for scores >= 70
    risk_multiplier_medium_score: float = 1.0
 # Multiplier for scores >= 50
    risk_multiplier_low_score: float = 0.5
    # Multiplier for scores < 50
    min_stop_atr_ratio: float = 0.5
     # Minimum stop distance as ratio of ATR
    max_stop_atr_ratio: float = 4.0
     # Maximum stop distance as ratio of ATR
    kelly_fraction_conservative: float = 0.25
 # Conservative Kelly fraction cap
    volatility_parity_enabled: bool = False
  # Enable volatility parity sizing
    lot_size_enforcement: bool = True
   # Enforce NSE lot size constraints
    min_lot_size: int = 1
              # Minimum lot size (shares)
    default_lot_size: int = 1
          # Default lot size for unlisted symbols
    max_position_concentration: float = 0.05
  # Max 5% in single position for high vol
    liquidity_check_enabled: bool = True
     # Enable liquidity-based sizing
    min_avg_volume_multiple: float = 0.01
    # Position size as % of avg volume
    # Performance & Monitoring
    benchmark_symbol: str = "^NSEI"
     # NIFTY 50 as benchmark
    performance_lookback_days: int = 252
 # 1 year for performance metrics
    # File Paths & Output
    ...
    def validate(self) -> None:
        ...
        if max_theoretical_exposure > 2.0:
  # Allow up to 200% theoretical exposure (with margin)
            errors.append(
                f"max_position_size ({self.max_position_size}) * "
                f"max_positions ({self.max_positions}) = {max_theoretical_exposure:.1f} "
                "exceeds reasonable exposure limit (200%)"
            )
