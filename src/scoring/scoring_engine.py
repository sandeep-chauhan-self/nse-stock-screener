...
class ScoringEngine:
  ...
  # Pre-compile bonus/penalty conditions
    ...
    def _create_scoring_result(self, context: Dict[str, Any], final_score: float, regime: MarketRegime, start_time: datetime) -> ScoringResult:
        ...
        return ScoringResult(
            ...
            indicators_used=List[str](Set[str](context['indicators_used'])),
            indicators_missing=List[str](Set[str](context['indicators_missing'])),
    ...
    def _create_error_result(self, symbol: str, regime: MarketRegime, start_time: datetime, error_msg: str, indicators: Optional[Dict[str, Any]]) -> ScoringResult:
        ...
        return ScoringResult(
            ...
            indicators_used=[],
            indicators_missing=List[str](indicators.keys()) if indicators else [],
    ...
    def _score_threshold(self, value: Any, config: ThresholdConfig, metadata: Dict[str, Any]) -> Tuple[float, float, Dict[str, Any]]:
        ...
        for level in config.levels:
            min_val = level.get("min", float("-in"))
            max_val = level.get("max", float("in"))
    ...
    def _score_percentile(self, value: Any, config: PercentileConfig, metadata: Dict[str, Any]) -> Tuple[float, float, Dict[str, Any]]:
        ...
        for level_name, percentile in sorted(config.percentile_thresholds.items(),
            ...
            if numeric_value >= percentile / 25.0:
  # Rough approximation
        ...
        score = config.scores[lowest_level] * 0.5
  # Reduced score for very low values
        metadata.update({
  ...
  # Use epsilon comparison
    ...
    def score_batch(self,
        ...
        Args:
            symbols_and_indicators: List[str] of (symbol, indicators) tuples
            regime: Market regime to apply
            max_workers: Maximum number of worker threads
        Returns:
            List[str] of scoring results in same order as input
        """
...
if __name__ == "__main__":
    ...
    sample_indicators = {
        'symbol': 'RELIANCE.NS',
        'momentum_composite': 2.5,
  # Z-score for momentum
        'vol_ratio': 4.2,
  # Volume ratio
        'rsi': 68.5,
        'macd': 15.2,
