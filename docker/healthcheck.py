...
# Setup logging
logging.basicConfig(
    # Quiet by default for health checks
...
class HealthChecker(object):
    ...
    def add_check(self, name: str, status: bool, message: str = "", details: Optional[Dict[str, Any]] = None) -> None:
    ...
    def check_secrets_management(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                # Handle both SecretValidationError and other exceptions
                self.add_check(
                    "secrets_management",
                    False,
    ...
    def check_security_posture(self) -> bool:
        ...
        try:
            security_issues = []
            # Check if running as root (security risk) - Unix only
            try:
                # Use getattr to safely check for getuid on Unix systems
                getuid_func = getattr(os, 'getuid', None)
                if getuid_func is not None and getuid_func() == 0:
                    security_issues.append("Running as root user")
            except (AttributeError, OSError):
                # Windows doesn't have getuid or other OS errors
                pass
    ...
    def get_health_report(self) -> Dict[str, Any]:
        ...
        # Get user ID safely across platforms
        user_id = -1
        try:
            getuid_func = getattr(os, 'getuid', None)
            if getuid_func is not None:
                user_id = getuid_func()
        except (AttributeError, OSError):
            user_id = -1  # Not available on Windows or other platforms
...
def main() -> None:
    ...
    try:
        ...
        if args.json:
            ...
            print("NSE Stock Screener Health Check")
