...
This module contains carefully crafted test data with known expected outputs
for validating indicators, backtesting, and risk management functionality.
"""
...
def create_test_ohlcv_data() -> pd.DataFrame:
    ...
    for i, close in enumerate(closes):
        # 2.0%, 2.5%, 3.0% rotating
        range_pct = 0.02 + (i % 3) * 0.005
...
def create_expected_indicator_results() -> dict[str, Any]:
    ...
    return {
        ...
        # After initial uptrend (approximately)
        'rsi_day_15': 67.89,
        # During strong uptrend
        'rsi_day_25': 85.32,
        # During strong downtrend
        'rsi_day_35': 15.67,
        # During sideways movement
        'rsi_day_45': 52.13,
        # ATR values (14-period ATR)
        # Should be around this based on our range design
        'atr_day_15': 2.45,
        'atr_day_25': 2.58,
        'atr_day_35': 2.67,
        'atr_day_45': 2.42,
        # Volume statistics
        # Average volume in first 20 days
        'avg_volume_first_20': 125000,
        # Maximum volume in dataset
        'max_volume': 200000,
        # Minimum volume in dataset
        'min_volume': 100000,
        # Price statistics
        # Approximately, after uptrend
        'max_close': 133.7,
        # Approximately, after downtrend
        'min_close': 105.2,
        # Final closing price
        'final_close': 105.7,
        # MACD approximations (12,26,9)
        # Approximate day of bullish crossover
        'macd_bullish_crossover_day': 18,
        # Approximate day of bearish crossover
        'macd_bearish_crossover_day': 32,
        # ADX trend strength
        # Expected ADX during trend periods
        'adx_trend_strength': 25.5,
    }
def create_regime_test_data() -> dict[str, pd.DataFrame]:
    ...
    sideways_prices = [120 + np.sin(i * 0.4) * 3 + rng.normal(0, 0.2) for i in range(30)]
...
def create_risk_management_test_scenarios() -> dict[str, dict[str, Any]]:
    ...
    return {
        'normal_trade': {
            ...
            'stop_loss': 95.0,
            'risk_per_trade': 0.01,
            # 1% of 100k = 1000, divided by 5 risk = 200 shares
        ...
        'high_risk_trade': {
            ...
            'stop_loss': 180.0,
            'risk_per_trade': 0.02,
            # 2% of 100k = 2000, divided by 20 risk = 100 shares
        ...
        'small_stop_trade': {
            ...
            'stop_loss': 49.0,
            'risk_per_trade': 0.01,
            # 1% of 100k = 1000, divided by 1 risk = 1000 shares
